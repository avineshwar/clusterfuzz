...
class SeverityAnalyzerTest(unittest.TestCase):
    ...
    def test_asan_uaf(self):
        ...
        self.assertEqual(
            severity_analyzer.get_analyzer("sanitizer_generic").analyze(
                "Heap-use-after-free\nREAD 8",
                self._read_test_data("asan_uaf.txt"),
                False,
            ),
    ...
    def test_asan_heap_overflow_read(self):
        ...
        self.assertEqual(
            severity_analyzer.get_analyzer("sanitizer_generic").analyze(
                ...
                self._read_test_data("asan_heap_overflow_read.txt"),
                False,
            ),
            SecuritySeverity.MEDIUM,
        )
    ...
    def test_asan_heap_overflow_write(self):
        ...
        self.assertEqual(
            severity_analyzer.get_analyzer("sanitizer_generic").analyze(
                "Heap-buffer-overflow\nWRITE 16",
                self._read_test_data("asan_heap_overflow_write.txt"),
                False,
            ),
            SecuritySeverity.HIGH,
        )
    ...
    def test_asan_container_overflow_read(self):
        ...
        self.assertEqual(
            severity_analyzer.get_analyzer("sanitizer_generic").analyze(
                "Container-overflow\nREAD 4",
                self._read_test_data("asan_container_overflow_read.txt"),
                False,
            ),
            SecuritySeverity.MEDIUM,
        )
    ...
    def test_asan_unknown_read(self):
        ...
        self.assertEqual(
            severity_analyzer.get_analyzer("sanitizer_generic").analyze(
                "UNKNOWN READ", self._read_test_data("asan_unknown_read.txt"), False
            ),
            SecuritySeverity.MEDIUM,
        )
    ...
    def test_asan_unknown_write(self):
        ...
        self.assertEqual(
            severity_analyzer.get_analyzer("sanitizer_generic").analyze(
                "UNKNOWN WRITE", self._read_test_data("asan_unknown_write.txt"), False
            ),
            SecuritySeverity.HIGH,
        )
    ...
    def test_tsan_uaf(self):
        ...
        self.assertEqual(
            severity_analyzer.get_analyzer("sanitizer_generic").analyze(
                ...
                self._read_test_data("tsan_use_after_free.txt"),
                False,
            ),
            SecuritySeverity.HIGH,
        )
    ...
    def test_ubsan_bad_cast(self):
        ...
        self.assertEqual(
            severity_analyzer.get_analyzer("sanitizer_generic").analyze(
                "Bad-cast", self._read_test_data("ubsan_bad_cast_downcast.txt"), False
            ),
            SecuritySeverity.HIGH,
        )
    ...
    def test_ubsan_object_size(self):
        ...
        self.assertEqual(
            severity_analyzer.get_analyzer("sanitizer_generic").analyze(
                "Object-size", self._read_test_data("ubsan_object_size.txt"), False
            ),
            SecuritySeverity.MEDIUM,
        )
    ...
    def test_msan_renderer(self):
        ...
        self.assertEqual(
            severity_analyzer.get_analyzer("sanitizer_chrome").analyze(
                ...
                self._read_test_data("msan_renderer.txt"),
                False,
            ),
            SecuritySeverity.MEDIUM,
        )
    ...
    def test_msan_browser(self):
        ...
        self.assertEqual(
            severity_analyzer.get_analyzer("sanitizer_chrome").analyze(
                "Use-of-uninitialized-value",
                self._read_test_data("msan_browser.txt"),
                False,
            ),
            SecuritySeverity.MEDIUM,
        )
    ...
    def test_find_process_type_browser(self):
        ...
        self.assertEqual(
            severity_analyzer.get_analyzer("sanitizer_chrome").analyze(
                "Heap-use-after-free\nREAD 8",
                self._read_test_data("browser_uaf.txt"),
                False,
            ),
    ...
    def test_find_process_type_browser_2(self):
        ...
        self.assertEqual(
            severity_analyzer.get_analyzer("sanitizer_chrome").analyze(
                "Heap-use-after-free\nREAD 8",
                self._read_test_data("browser_uaf2.txt"),
                False,
            ),
            SecuritySeverity.CRITICAL,
        )
    ...
    def test_find_process_type_browser_3(self):
        ...
        self.assertEqual(
            severity_analyzer.get_analyzer("sanitizer_chrome").analyze(
                "Heap-use-after-free\nREAD 8",
                self._read_test_data("browser_uaf3.txt"),
                False,
            ),
            SecuritySeverity.CRITICAL,
        )
