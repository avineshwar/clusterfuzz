...
class ComponentPath(object):
    ...
    def __eq__(self, other):
        return (
            ...
            and self.display_path == other.display_path
        )
...
class VCSViewer(object):
    ...
    def get_source_url_for_revision_path_and_line(self, revision, path, line):
        ...
        return self.get_mapped_url(
            self.VCS_REVISION_PATH_LINE_SUB, revision=revision, path=path, line=line
        )


class FreeDesktopVCS(VCSViewer):
    VCS_URL_REGEX = re.compile(r"https://anongit\.freedesktop\.org/git/(.*)\.git$")
    VCS_REVISION_SUB = r"https://cgit.freedesktop.org/\1/commit/?id={revision}"
    VCS_REVISION_DIFF_SUB = (
        r"https://cgit.freedesktop.org/\1/diff/"
...
class GoogleSourceVCS(VCSViewer):
    VCS_URL_REGEX = re.compile(r"(https://[^/]+\.googlesource\.com/(.*?))(\.git)?$")
...
class MercurialVCS(VCSViewer):
    ...
    VCS_REVISION_DIFF_SUB = (
        r"\1/log?rev={start_revision}%3A%3A{end_revision}" r"&revcount={range_limit}"
    )
...
def get_component_source_and_relative_path(path, revisions_dict):
    ...
    for component_source in component_sources:
        ...
        # which is incorrect.
        stripped_component_source = SOURCE_STRIP_REGEX.sub("", component_source) + "/"

        if normalized_path.startswith(stripped_component_source):
            relative_path = utils.strip_from_left(
                normalized_path, stripped_component_source
            )
    ...
    return ComponentPath(default_component_source, normalized_path, normalized_path)
...
def linkify_stack_frame(stack_frame, revisions_dict):
    ...
    linkified_stack_frame = STACK_FRAME_PATH_LINE_REGEX.sub(link_html, stack_frame)
