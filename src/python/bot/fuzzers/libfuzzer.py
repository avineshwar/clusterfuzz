...
class AndroidLibFuzzerRunner(new_process.UnicodeProcessRunner, LibFuzzerCommon):
    ...
    def run_single_testcase(self, testcase_path, timeout=None, additional_args=None):
        ...
        with self._device_file(testcase_path) as device_testcase_path:
            result = LibFuzzerCommon.run_single_testcase(
...
def fix_timeout_argument_for_reproduction(arguments):
    ...
    adjusted_test_timeout = max(
        1, environment.get_value("TEST_TIMEOUT") - constants.REPORT_PROCESSING_TIME
    )
...
def pick_strategies(
    ...
    use_dataflow_tracing = (
        ...
        strategy.ENTROPIC_STRATEGY.name not in fuzzing_strategies
        and strategy_pool.do_strategy(strategy.DATAFLOW_TRACING_STRATEGY)
    )
