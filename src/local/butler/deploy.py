...
def _update_alerts(project):
    ...
    alerts_path = os.path.join(
        environment.get_config_directory(), "monitoring", "alerts.yaml"
    )
...
def _update_bigquery(project):
    ...
    _update_deployment_manager(
        project,
        "bigquery",
        os.path.join(environment.get_config_directory(), "bigquery", "datasets.yaml"),
    )
...
def _update_redis(project):
    ...
    _update_deployment_manager(
        project,
        "redis",
        os.path.join(environment.get_config_directory(), "redis", "instance.yaml"),
    )
