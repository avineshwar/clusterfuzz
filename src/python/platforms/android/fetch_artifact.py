...
def download_artifact(
    client, bid, target, attempt_id, name, output_directory, output_filename
):
    ...
    if artifact is None:
        logs.log_error(
            "No artifact found with name %s, target %s and build id %s."
            % (name, target, bid)
        )
        return False
...
def get_client():
    ...
    if not build_apiary_service_account_private_key:
        logs.log("Android build apiary credentials are not set, skip artifact fetch.")
        return None
...
def get_latest_artifact_info(branch, target, signed=False):
    ...
    if not builds:
        logs.log_error("No artifact found for target %s, branch %s." % (target, branch))
        return None
...
def run_script(client, bid, target, regex, output_directory, output_filename):
    ...
    if not artifacts:
        logs.log_error("No artifact found for target %s, build id %s." % (target, bid))
        return False
