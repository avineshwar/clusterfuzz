...
def _save_fixed_range(testcase_id, min_revision, max_revision, testcase_file_path):
...
def _store_testcase_for_regression_testing(testcase, testcase_file_path):
    ...
    regression_testcase_url = os.path.join(
        corpus.get_regressions_corpus_gcs_url(), utils.file_hash(testcase_file_path)
    )

    if storage.copy_file_to(testcase_file_path, regression_testcase_url):
        logs.log(
            "Successfully stored testcase for regression testing: "
            + regression_testcase_url
        )
    else:
        logs.log_error(
            "Failed to store testcase for regression testing: "
            + regression_testcase_url
        )
...
def find_fixed_range(testcase_id, job_type):
    ...
    while time.time() < deadline:
        ...
        if max_index - min_index == 1:
            _save_fixed_range(
                testcase_id, min_revision, max_revision, testcase_file_path
            )
            return
