...
class GetDirectoryFunctionsTest(unittest.TestCase):
    ...
    def test_get_mutator_plugins_archives_dir(self):
        ...
        self.assertEqual(
            os.path.join(self.plugins_root_dir, mutator_plugin.ARCHIVES_SUBDIR_NAME),
...
class PluginGetterTest(fake_filesystem_unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.plugin_archive_path = os.path.join(
            self.plugins_archives_dir, self.plugin_archive_filename
        )
        ...
        def mocked_copy_file_from(gcs_url, file_path):
            expected_url = "%s/%s" % (
                mutator_plugin._get_mutator_plugins_bucket_url(),
...
class ExtractNameFromArchiveTest(unittest.TestCase):
    ...
    def test_extract_name_from_archive(self):
        ...
        plugin_archive_filename = "%s-%s-%s.zip" % (name, job_name, fuzzer_binary_name)
        extracted_name, job_and_fuzz_target = mutator_plugin._extract_name_from_archive(
