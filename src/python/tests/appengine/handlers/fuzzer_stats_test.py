...
def _mock_query(fuzzer, jobs, group_by, date_start, date_end):
  return json.loads(
      _read_data_file("%s_%s_%s_%s_%s.txt" %
                      (fuzzer, "_".join(jobs) if jobs else None, group_by,
                       date_start, date_end)))
...
class TestPermissions(unittest.TestCase):
  ...
  def test_external_user_without_job(self):
    ...
    self.assertEqual(200, response.status_int)
    self.mock.build_results.assert_called_with("fuzzer", ["job1", "job2"],
                                               "by-fuzzer", "2017-01-01",
                                               "2017-01-07")
