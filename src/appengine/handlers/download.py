...
class Handler(base_handler.Handler, gcs.SignedGcsHandler):
    ...
    def _send_blob(
        self, blob_info, testcase_id, is_minimized=False, fuzzer_binary_name=None
    ):
        """Send the blob."""
        minimized_string = "minimized-" if is_minimized else ""
        fuzzer_binary_string = fuzzer_binary_name + "-" if fuzzer_binary_name else ""
        if testcase_id:
        ...
        response = self.serve_gcs_object(
    ...
    def get(self, resource=None):
        ...
        if (
            testcase
            and testcase.fuzzed_keys != blob_info.key()
        ...
        if access.has_access():
            ...
            return self._send_blob(
                blob_info, testcase_id, is_minimized, fuzzer_binary_name
            )
