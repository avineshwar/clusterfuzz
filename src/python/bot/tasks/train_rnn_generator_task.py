...
def get_last_saved_model(model_directory):
    ...
    file_pattern = os.path.join(model_directory, "*" + constants.MODEL_INDEX_SUFFIX)
...
def upload_model_to_gcs(model_directory, fuzzer_name):
    ...
    logs.log(
        "Uploading the model for %s: %s, %s."
        % (fuzzer_name, data_file_name, index_file_name)
    )

    # Upload files to GCS.
    result = storage.copy_file_to(
        latest_data_file, gcs_data_path
