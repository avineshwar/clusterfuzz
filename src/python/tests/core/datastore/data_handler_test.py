...
class DataHandlerTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        entities_to_put = [
            ...
            self.job,
            self.job2,
            self.local_data_bundle,
            self.cloud_data_bundle,
            self.fuzzer1,
            self.fuzzer2,
            self.fuzzer3,
        ]
    ...
    def test_find_testcase(self):
        ...
        # type.
        self.assertIsNone(
            data_handler.find_testcase(
                "project", crash_type, "missing_state", security_flag
            )
        )
        self.assertIsNone(
            data_handler.find_testcase(
                "project", "missing type", crash_state, security_flag
            )
        )

        # Ensure that we respect security flag when searching.
        result = data_handler.find_testcase("project", crash_type, crash_state, False)
        ...
        self.assertTrue(test_utils.entities_equal(exclude_result, reproducible_no_bug))
    ...
    def test_get_issue_summary_bad_cast(self):
        ...
        self.assertEqual(
            summary,
            "project: Bad-cast to blink::LayoutBlock from "
    ...
    def test_get_issue_summary_bad_cast_without_crash_function(self):
        ...
        self.assertEqual(
            summary,
            "project: Bad-cast to blink::LayoutBlock from " "blink::LayoutTableSection",
        )

    def test_get_data_bundle_name_default(self):
        """Test getting the default data bundle bucket name."""
        self.assertEqual(
            "test-corpus.test-clusterfuzz.appspot.com",
...
class GetIssueTrackerNameTest(unittest.TestCase):
    ...
    def test_get_default(self):
        ...
        self.assertEqual(
            "test-issue-tracker", data_handler.get_issue_tracker_name("job")
        )
...
class GetProjectNameTest(unittest.TestCase):
    ...
    def test_get_from_job(self):
        ...
        data_types.Job(
            name="job",
            environment_string=(
                "PROJECT_NAME = from_internal_project\n" "HELP_URL = help_url\n"
            ),
        ).put()
        self.assertEqual("from_internal_project", data_handler.get_project_name("job"))
...
class GetSecuritySeverityTest(unittest.TestCase):
    ...
    def setUp(self):
        helpers.patch(self, ["crash_analysis.severity_analyzer.get_security_severity",])
        self.gestures = ""
    ...
    def test_get(self):
        ...
        self.mock.get_security_severity.assert_called_with(
            "type", "trace", "job", False
        )
...
class UpdateTestcaseCommentTest(unittest.TestCase):
    ...
    def setUp(self):
        helpers.patch_environ(self)
        helpers.patch(self, ["base.utils.current_date_time",])
    ...
    def test_update_comment_truncate(self):
        ...
        self.assertEqual(
            data_types.TESTCASE_COMMENTS_LENGTH_LIMIT, len(self.testcase.comments)
        )
        expected_new = "[2019-01-01 00:00:00] bot: Progression task started: message.\n"
        expected = (
