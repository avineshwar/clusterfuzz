...
class HandlerTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        data_types.Job(
            name="libfuzzer_msan", environment_string="USE_CORPUS_FOR_ML = True\n"
        ).put()
        data_types.Job(
            name="afl_asan", environment_string="USE_CORPUS_FOR_ML = False\n"
        ).put()
        ...
        # Create fake fuzzers.
        data_types.Fuzzer(
            name="libFuzzer", jobs=["libfuzzer_asan", "libfuzzer_asan_gradientfuzz"]
        ).put()
        data_types.Fuzzer(name="afl", jobs=["afl_asan"]).put()

        # Create fake child fuzzers.
        data_types.FuzzTarget(
            engine="libFuzzer", binary="fake_fuzzer", project="test-project"
        ).put()
        data_types.FuzzTargetJob(
            fuzz_target_name="libFuzzer_fake_fuzzer", job="libfuzzer_asan"
        ).put()
        data_types.FuzzTarget(
            engine="afl", binary="fake_fuzzer", project="test-project"
        ).put()
        data_types.FuzzTargetJob(
            fuzz_target_name="afl_fake_fuzzer", job="afl_asan"
        ).put()

        data_types.FuzzTarget(
            engine="libFuzzer", binary="fake_gradientfuzzer", project="test-project"
        ).put()
        data_types.FuzzTargetJob(
            fuzz_target_name="libFuzzer_fake_gradientfuzzer",
            job="libfuzzer_asan_gradientfuzz",
        ).put()

        data_types.FuzzTarget(
            engine="libFuzzer", binary="fake_all_fuzzer", project="test-project"
        ).put()
        data_types.FuzzTargetJob(
            fuzz_target_name="libFuzzer_fake_all_fuzzer", job="libfuzzer_asan_all"
        ).put()
