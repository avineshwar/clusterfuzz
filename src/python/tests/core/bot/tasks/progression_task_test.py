...
class WriteToBigqueryTest(unittest.TestCase):
    ...
    def setUp(self):
        helpers.patch(self, ["google_cloud_utils.big_query.write_range",])
    ...
    def test_write(self):
        """Tests write."""
        progression_task._write_to_bigquery(
            self.testcase, 456, 789
...
class UpdateIssueMetadataTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.testcase = data_types.Testcase(overridden_fuzzer_name="libFuzzer_fuzzer")
        self.testcase.put()

    def test_update_issue_metadata_non_existent(self):
        """Test update issue metadata a testcase with no metadata."""
        progression_task._update_issue_metadata(
            self.testcase
        )  # pylint: disable=protected-access

        testcase = self.testcase.key.get()
        self.assertDictEqual(
            {"issue_labels": "label1", "issue_components": "component1",},
    ...
    def test_update_issue_metadata_replace(self):
        ...
        self.testcase.additional_metadata = json.dumps(
            {"issue_labels": "label1", "issue_components": "component2",}
        )
        progression_task._update_issue_metadata(
            self.testcase
        )  # pylint: disable=protected-access
...
class StoreTestcaseForRegressionTesting(fake_filesystem_unittest.TestCase):
    ...
    def setUp(self):
        ...
        helpers.patch(self, ["google_cloud_utils.storage.copy_file_to",])
