...
class JobsTest(unittest.TestCase):
    ...
    def test_post(self):
        ...
        self.assertListEqual(
            expected_items[1], [item["id"] for item in resp.json["items"]]
        )

        resp = self.app.post_json("/", {"page": 2})
        self.assertListEqual(
            expected_items[2], [item["id"] for item in resp.json["items"]]
        )

        resp = self.app.post_json("/", {"page": 3})
        self.assertListEqual(
            expected_items[3], [item["id"] for item in resp.json["items"]]
        )
    ...
    def test_fuzzers_result(self):
        ...
        resp = self.app.post_json("/", {"page": 1})
        self.assertListEqual(
            [job.key.id()], [item["id"] for item in resp.json["items"]]
        )
...
class JobsSearchTest(unittest.TestCase):
    ...
    def test_post(self):
        ...
        self.assertListEqual(
            [job_asan.key.id()], [item["id"] for item in resp.json["items"]]
        )

        resp = self.app.post_json("/", {"q": "ubsan"})
        self.assertListEqual(
            [job_ubsan.key.id()], [item["id"] for item in resp.json["items"]]
        )
...
class JobsUpdateTest(unittest.TestCase):
    ...
    def test_post(self):
        ...
        self.mock.update_mappings_for_job.assert_called_with(mock.ANY, ["test_fuzzer"])
