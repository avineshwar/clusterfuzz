...
def _mock_query(fuzzer, jobs, group_by, date_start, date_end):
    return json.loads(
        _read_data_file(
            "%s_%s_%s_%s_%s.txt"
            % (fuzzer, "_".join(jobs) if jobs else None, group_by, date_start, date_end)
        )
    )
...
class TestBuildResults(unittest.TestCase):
    ...
    def setUp(self):
        ...
        data_types.Fuzzer(
            name="testFuzzer",
            stats_columns=(
                "sum(t.blah) as blah, custom(j.new_crashes) "
        ...
        data_types.Job(name="job", environment_string="FUZZ_LOGS_BUCKET = bucket").put()

        now = datetime.datetime.utcnow()

        data_types.FuzzTarget(
            engine="testFuzzer", project="test-project", binary="1_fuzzer"
        ).put()
        data_types.FuzzTargetJob(
            fuzz_target_name="testFuzzer_1_fuzzer", job="job", last_run=now
        ).put()

        data_types.FuzzTarget(
            engine="testFuzzer", project="test-project", binary="2_fuzzer"
        ).put()
        data_types.FuzzTargetJob(
            fuzz_target_name="testFuzzer_2_fuzzer", job="job", last_run=now
        ).put()

        data_types.FuzzTarget(
            engine="testFuzzer", project="test-project", binary="3_fuzzer"
        ).put()
        data_types.FuzzTargetJob(
            fuzz_target_name="testFuzzer_3_fuzzer", job="job", last_run=now
        ).put()

        data_types.FuzzTarget(
            engine="testFuzzer2", project="test-project", binary="1_fuzzer"
        ).put()
        data_types.FuzzTargetJob(
            fuzz_target_name="testFuzzer2_1_fuzzer", job="job", last_run=now
        ).put()
        ...
        test_helpers.patch(self, ["google_cloud_utils.big_query.Client",])
    ...
    def test_build_by_fuzzer(self):
        ...
        build_args = ["testFuzzer", ["job"], "by-fuzzer", "2016-10-20", "2016-10-21"]
    ...
    def test_build_none_results(self):
        ...
        self.assertDictEqual(
            ast.literal_eval(_read_data_file("by_fuzzer_expected_None.txt")), result
        )
    ...
    def test_build_today_results(self):
        ...
        build_args = ["testFuzzer_2_fuzzer", ["job"], "by-time", date_today, date_today]
...
class TestPermissions(unittest.TestCase):
    ...
    def test_internal_user_with_job(self):
        ...
        self.mock.build_results.assert_called_with(
            "fuzzer", ["job1"], "by-fuzzer", "2017-01-01", "2017-01-07"
        )
    ...
    def test_internal_user_without_job(self):
        ...
        self.assertEqual(200, response.status_int)
        self.mock.build_results.assert_called_with(
            "fuzzer", None, "by-fuzzer", "2017-01-01", "2017-01-07"
        )
