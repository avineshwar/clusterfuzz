...
LINUX_GDB_CRASH_ADDRESS_NO_REGISTERS_REGEX = re.compile(r"^(0[xX][0-9a-fA-F]+)\s+in\s+")
...
RUST_ASSERT_REGEX = re.compile(r"thread\s.*\spanicked at \'([^\']*)", re.IGNORECASE)
...
STACK_FRAME_IGNORE_REGEXES = [
    ...
    r"^abort$",
    r"^exit$",
    r"^pthread\_create$" r"^pthread\_kill$",
    r"^raise$",
    r"^tgkill$",
    ...
    r"^android\.os\.Parcel\.",
    r"^art\:\:Thread\:\:CreateNativeThread" r"^asan\_",
    r"^calloc",
    ...
    if environment.is_android() and (found_android_kernel_crash or is_kasan):
        ...
        if android_kernel_prefix and android_kernel_hash:
            ...
            for line in state.crash_stacktrace.splitlines():
                temp_crash_stacktrace += (
                    android_kernel.get_kernel_stack_frame_link(
