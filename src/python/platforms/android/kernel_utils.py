...
def get_kernel_stack_frame_link(stack_frame, kernel_prefix, kernel_hash):
    ...
    kernel_url_info = (
        r"http://go/pakernel/{prefix}/+/{hash}/{path}#{line};" r"{display_path};"
    ).format(
...
def get_kernel_prefix_and_full_hash():
    ...
    symbols_directory = os.path.join(environment.get_value("SYMBOLS_DIR"), "kernel")
    ...
    repro_filename = symbols_downloader.get_symbols_archive_filename(build_id, target)
    ...
    if local_repo_path and os.path.exists(local_repo_path):
        android_kernel_repo_data = utils.read_data_from_file(
            local_repo_path, eval_data=False
        ).decode()
        return _get_prefix_and_full_hash(android_kernel_repo_data, kernel_partial_hash)
